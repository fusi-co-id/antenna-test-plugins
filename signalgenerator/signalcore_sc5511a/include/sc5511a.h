 ...
 *	
 *	Copyright (c) 2013-2022 SignalCore Inc.
 *	
 * Rev 2.2
 *
 * Rev Notes:	
 *				(1) Changes only applies to hardware revision > 14
 *				(2) Added sc5511a_get_clock_config() to retrive just the reference clock parameters
 *				(3) Modified sc5511a_set_clock_reference() with 2 new parameters to set ext_ref_freq and ext_direct_clk
 *				
 ****************************************************************************
 * SC5511A header file 
*/
...
typedef struct
{
	unsigned char ext_ref_lock_enable;	// select to lock to external reference source	 
	unsigned char ref_out_select;		// select either 10 or 100 MHz output
#ifdef PXI
	unsigned char pxi_clock_enable;
#endif
	unsigned char ext_direct_clocking;	// enable direct 100 MHz clocking of the synthesiszer, bypassing its internal 100 MHz clock.
	unsigned char ext_ref_freq;			// selects the input frequency
} clock_config_t;
...
/*	Function opens the target USB device.
	...
	input: 		devSerialNum is the product serial number. Product number is available on
				the product label.
*/
EXPORT_DLL int sc5511a_open_device_m(char* dev_serial_num, sc5511a_device_handle_t *device_handle);
...
/** set the reference clock behavior
	...
	input:	lock_eternal: 1 locks the 100 MHz reference to the external 10 MHz source
			...
			ext_direct_clk: 1 expects a 100MHz reference to directly clock the synthesizer, bypassing its internal 100 MHz
			ext_ref_freq:	selects 10 or 100 MHz to lock to.
*/
EXPORT_DLL int sc5511a_set_clock_reference(sc5511a_device_handle_t dev_handle, unsigned char ext_ref_freq, unsigned char ext_direct_clk,
					unsigned char select_high, unsigned char lock_external);
...
/*	Function retrives current phase of the signal on ch1
return:	error code
output:	phase
*/
EXPORT_DLL int sc5511a_get_signal_phase(sc5511a_device_handle_t dev_handle, float *phase);
...
/*	This function works only for devices with firmware >= ver3.6 and hardware > ver16.0
	...
	output:	device status
*/
EXPORT_DLL int sc5511a_get_clock_config(sc5511a_device_handle_t dev_handle, clock_config_t* clock_config);
